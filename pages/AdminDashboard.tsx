...
const AdminDashboard: React.FC = () => {
  ...
  const [viewingMapAddress, setViewingMapAddress] = useState<string | null>(null);
  ...
  return (
    <div className="min-h-screen bg-slate-950 pt-20 pb-12 px-4 relative">
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {filteredBookings.map((booking) => (
                      <tr key={booking.id} className="hover:bg-slate-800/50 transition-colors">
                        <td className="p-4">
                          <div className="font-bold text-white mb-1">{booking.userName}</div>
                          <a 
                            href={`tel:${booking.phone}`}
                            className="text-sm text-amber-400 hover:text-amber-300 flex items-center gap-1 mb-2 font-medium w-fit"
                            title="Call Customer"
                          >
                            <Phone size={12} /> {booking.phone}
                          </a>
                          <div className="flex items-start gap-2">
                            <div className="text-xs text-gray-500 max-w-[200px] truncate" title={booking.address}>
                              {booking.address}
                            </div>
                            <button 
                              onClick={() => setViewingMapAddress(booking.address)}
                        ...
                        <td className="p-4 text-right">
                          <div className="flex justify-end gap-2">
                            <a
                              href={`tel:${booking.phone}`}
                              className="p-2 bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 rounded-lg border border-amber-500/30"
                              title="Call Customer"
                            >
                              <Phone size={18} />
                            </a>
            ...
            {messages.map((msg) => (
              <div key={msg.id} className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-slate-600 transition-all shadow-lg">
                ...
                <div className="mt-4 flex justify-end">
                  <a 
                    href={`tel:${msg.phone}`}
                    className="text-xs bg-white text-black px-3 py-2 rounded font-bold hover:bg-gray-200 transition-colors flex items-center gap-1"
                  >
      ...
      {/* Map Modal */}
      {viewingMapAddress && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 p-2 md:p-4 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-slate-900 w-[95vw] h-[90vh] rounded-2xl border border-slate-700 shadow-2xl overflow-hidden flex flex-col">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-800">
              <h3 className="text-white font-bold flex items-center gap-2">
                <MapPin className="text-amber-500" size={20}/> Customer Location
              </h3>
              <button 
                onClick={() => setViewingMapAddress(null)} 
            ...
            <div className="flex-grow bg-slate-800 relative w-full h-full">
               <iframe 
                 width="100%" 
                 height="100%" 
                 frameBorder="0" 
                 scrolling="no" 
                 src={`https://maps.google.com/maps?q=${encodeURIComponent(viewingMapAddress)}&t=m&z=17&output=embed`}
                 title="Customer Location"
                 className="w-full h-full"
               ></iframe>
            </div>
            
            <div className="p-4 bg-slate-900 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4">
               <div className="flex items-center gap-2 text-sm text-gray-300 overflow-hidden">
                 <MapPin size={16} className="text-gray-500 flex-shrink-0" />
                 <span className="truncate font-mono">{viewingMapAddress}</span>
               </div>
               <a 
                 href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(viewingMapAddress)}`} 
                 target="_blank" 
                 rel="noreferrer"
                 className="bg-white text-black px-6 py-3 rounded-lg text-sm font-bold hover:bg-gray-200 transition-colors flex items-center gap-2 whitespace-nowrap"
               >
